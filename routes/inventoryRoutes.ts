...
inventoryRoutes.route("/").get(async (req: Request, res: Response) => {
	try {
		...
		// Build WHERE clause if companyUuid is provided
	  ...
	  `;
		...
		// Fetch paginated inventory data with joins
		const query = `
		SELECT 
		  inventory.id,
		  inventory.uuid,
...
inventoryRoutes.route("/company/:companyUuid").get(async (req: Request, res: Response) => {
	try {
